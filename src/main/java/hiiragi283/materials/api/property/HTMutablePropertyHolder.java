...
public interface HTMutablePropertyHolder extends HTPropertyHolder {
  ...
  default <T> @Nullable T setPropertyIfNonNull(
      @NotNull final HTPropertyKey<T> key, @Nullable final T value) {
  ...
  default <T> @Nullable T removeIf(
      @NotNull final HTPropertyKey<T> key, @NotNull final Predicate<? super T> predicate) {
